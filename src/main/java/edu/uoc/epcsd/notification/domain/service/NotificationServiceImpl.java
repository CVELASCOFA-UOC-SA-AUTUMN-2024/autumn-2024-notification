...
public class NotificationServiceImpl implements NotificationService {
    ...
    public void notifyProductAvailable(ProductMessage productMessage) {
            GetProductResponse productResponse = new RestTemplate().getForEntity(productServiceUrl, GetProductResponse.class, productMessage.getProductId()).getBody();
        ...
        for (GetUserResponse user : usersToAlert) {
            log.info("Sending email to user " + user.getFullName() + " email " + user.getEmail() + " to notify that we have availability of the product " + productResponse.getName());
        }
    }
}
